function [f, grad] = iso_spatial_D2Lp_regularization(Knots, kspc, Nd, p, deps, edge_prior)
    ...
    for in = 1 : size(Knots, 5)
        ...
        for i = 1 : Nch
            ...
            if Nd == 3
                ...
                [Gt{1}, ~, ~] = my_gradient(Gt{1}, kspc);
                [~, Gt{2}, ~] = my_gradient(Gt{2}, kspc);
                [~, ~, Gt{3}] = my_gradient(Gt{3}, kspc);
            elseif Nd == 2
                [Gt{1}, Gt{2}] = my_gradient(tmpt, kspc(1:2));
                [Gt{1}, ~] = my_gradient(Gt{1}, kspc(1:2));
                [~, Gt{2}] = my_gradient(Gt{2}, kspc(1:2));
            end
            for j = 1 : Nd
                loc_tmp = Gt{j};
        ...
        for i = 1 : Nch
            if Nd == 3
                ...
                tmp_gr = my_gradient(my_gradient(t1, kspc, 1), kspc, 1)  + ...
                         my_gradient(my_gradient(t2, kspc, 2), kspc, 2)  + ...
                         my_gradient(my_gradient(t3, kspc, 3), kspc, 3);
                tmp_gr = reshape(tmp_gr, size(Knots(:,:,:, 1, in)));
                grad(:,:, :, i, in) = grad(:,:,:, i, in) + gvol * tmp_gr;
            elseif Nd == 2
                t1 = tmp_tg{i, 1} .* tmp_reg_2;
                t2 = tmp_tg{i, 2} .* tmp_reg_2;
                tmp_gr = my_gradient(my_gradient(t1, kspc(1:2), 1), kspc(1:2), 1)  + ...
                         my_gradient(my_gradient(t2, kspc(1:2), 2), kspc(1:2), 2);
