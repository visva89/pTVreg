function [t,x_new,f_new,g_new,funEvals,H] = ArmijoBacktrack(...
    cap_values, x,t,d,f,fr,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,saveHessianComp,funObj,varargin)
...
% plot(tdd(idxs), 'r-');
% hold on;
% plot(cap_values(idxs), 'b--');
% plot(min(abs(tdd(idxs)), cap_values(idxs)) .* sign(tdd(idxs)), 'k');
% pause(0.01);
% nnz(abs(t*d) > cap_values)/numel(d)

if ~isempty(cap_values)
    capped_xtd = @(x, td) x + min(abs(td), cap_values) .* sign(td);
else
    capped_xtd = @(x, td) x + td;
end

if nargout == 6
    [f_new,g_new,H] = funObj(capped_xtd(x, t*d),varargin{:});
else
    [f_new,g_new] = funObj(capped_xtd(x, t*d),varargin{:});
end
funEvals = 1;
