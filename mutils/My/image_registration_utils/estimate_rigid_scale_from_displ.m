function [t1, t2, alpha, scale, TransMat, Displ] = estimate_rigid_scale_from_displ(T, mask, varargin)
    ...
    opts = [];
    opts.display = 'off';
    objf = @(x) fgval(x(1), x(2), x(3), x(4), T, ND);
%     fl = @(x) x(:);
%     x0 = [mean(fl(T(:,:,1))); mean(fl(T(:,:,2))); 0];
    ...
    alpha = xmin(3) * pi / 180;
    scale = xmin(4);
    TransMat = [cos(alpha)*scale, -sin(alpha)*scale, t1; ...
                sin(alpha)*scale, cos(alpha)*scale, t2; ...
                0, 0, 1];
    Displ = displ_from_matrix_2d(TransMat, sz, [], true);
end
 
function f = fval(t1, t2, alpha, scale, T, ND)
...
%     pts0 = [pts0, ones(size(pts0,1), 1)]';
    ...
    ca = cos(alpha);
    sa = sin(alpha);
    R = [ca*scale, -sa*scale, t1; sa*scale, ca*scale, t2; 0, 0, 1];
%     ptsm = R * pts0;
...
function [f, g] = fgval(t1, t2, alpha, scale, T, ND)
    dx = 0.001;
    dx = 1e-6;
     
    f = fval(t1,t2,alpha, scale, T, ND);
    fp1 = fval(t1 + dx, t2, alpha, scale, T, ND);
    fp2 = fval(t1, t2 + dx, alpha, scale, T, ND);
    fp3 = fval(t1, t2, alpha + dx, scale, T, ND);
    fp4 = fval(t1, t2, alpha, scale + dx, T, ND);
    g = [(fp1 - f) / dx; (fp2 - f) / dx; (fp3 - f) / dx; (fp4 - f) / dx];
end