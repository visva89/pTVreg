function [D, szs] = generate_first_difference_matrix_for_image(sz, varargin)
...
if ndif == 1
    ...
    elseif numel(sz) == 3
        ...
        [m1, m2, m3] = ndgrid(1 : s1, 1 : s2, 1 : s3 - 1);
        tp = sub2ind(sz, m1(:), m2(:), m3(:));
        ...
        A3 = (sparse(1:K, tm, 1, K, P) - sparse(1:K, tp, 1, K, P)) / ...
            (spacing(3));
        A = [A1; A2; A3];
        szs = [size(A1, 1), size(A2, 1), size(A3, 1)];
    end
elseif ndif == 2
    if numel(sz) == 1
        e = ones(sz(1), 1);
        A = spdiags([e, -2*e, e], [0,1,2], sz(1), sz(1));
        A = A(1:end-2, :);
        A = A / spacing(1)^2;
        sz = 1;
    elseif numel(sz) == 2
        ...
        A2 = -2 * sparse(1:K, tm, 1, K, P) + sparse(1:K, tp1, 1, K, P) + ...
            sparse(1:K, tp2, 1, K, P);
        A = [A1/spacing(1)^2; A2/spacing(2)^2];
        szs = [size(A1, 1), size(A2, 1)];
    elseif numel(sz) == 3
        s1 = sz(1);
        s2 = sz(2);
