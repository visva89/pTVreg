...
for copd_idx = 1 : 10 
    ...
    TRE_e(copd_idx) = mean(TRE_phys);
    save('results/COPD_TRE_best.mat', 'TRE_e', 'TIME_e');
end
% idx = round(size(volfix,2)/2);
% imshowpair( [squeeze(volfix(:,idx, :))', squeeze(volfix(:,idx, :))'], [squeeze(volmov(:,idx,:))',squeeze(voldef(:,idx,:))'])